...
responseSummary = "hi"
def home(request):
    a = Author.objects.create(name='addie', email='sample@sample.com')
    a.save()

    sample = Author.objects.get(name='addie')
    print(sample)
...
def translate_pdf(pdf_name):
    user_lang = (
        input("What language would you like to translate your lease to? ")).lower()
    ...
    try:
        ...
        for s in pdf_sents:
            print('writing...')
            translated_file.write(client.translate(
                s, source_language='en', target_language=lang_code)['translatedText'])
...
def clarify_rights(violations, num_violations):
    ...
    ##########
    response += '(4) MAINTENANCE AND REPAIR'
    ...
    if violations['maintenance'] != []:
        response += 'Your lease may be neglecting to state the landlord’s duties.\n'
...
def main(url):
    ...
    print to new page summary string and response string
